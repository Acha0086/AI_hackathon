...
class agent:
    ...
    def next_move(self, game_state, player_state):
        ...
        #     (
        #         (self.opponent_bfs_graph[trap[0]][trap[1]] < 3) and \
        #         (self.me_bfs_graph[trap[0]][trap[1]] > self.opponent_bfs_graph[trap[0]][trap[1]])
        #     ):
        #     self.action = self.find_path_from_our_location(trap)  # OLD TRAP STRATEGY

        # if trap is not None:
        #     self.action = self.find_path_from_our_location(trap)
        # else:
        #     self.action = ''
        #     print("TRAP IS NONE")
        ...
        #     self.action = self.find_path_from_our_location(trap)  # OLD TRAP STRATEGY
        # elif trap is not None and \
        #     (self.opponent_bfs_graph[trap[0]][trap[1]] < 3 and self.me_bfs_graph[trap[0]][trap[1]] > self.opponent_bfs_graph[trap[0]][trap[1]]):
        #     self.action = self.find_path_from_our_location(trap)  # OLD TRAP STRATEGY
        elif trap is not None and \
            (self.opponent_bfs_graph[trap[0]][trap[1]] < 3):
            self.action = self.find_path_from_our_location(trap)  # OLD TRAP STRATEGY
        else:
            if len(self.game_state.treasure) > 0:
                treasure_location = self.find_agressive_resource("t")
