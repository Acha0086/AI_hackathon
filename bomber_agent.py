...
class agent:
    ...
    def next_move(self, game_state, player_state):
        ...
        self.opponent_location = game_state.opponents(player_state.id)[0] # gives list of tuples, only one opponent

        ammo = player_state.ammo
        bombs = game_state.bombs

        ## CALCULATE TRAP AND LOCATIONS
        blocks = self.game_state.blocks # THIS IS IN (x, y) FORM
        graph = self.convert_to_graph(blocks)
        bfs_graph, traps = self.get_dist_traps(graph, self.opponent_location)
        ext_trap_pos, min_trap_pos = self.get_ext_min(bfs_graph, traps)
    ...
    def check_trapped(self, ext_trap_pos, min_trap_pos, me_pos, enemy_pos):
